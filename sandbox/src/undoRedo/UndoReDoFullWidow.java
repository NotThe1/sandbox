...
public class UndoReDoFullWidow {
	...
	private AdapterUndoRedo adapterUndoRedo = new AdapterUndoRedo();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UndoReDoFullWidow window = new UndoReDoFullWidow();
	...
	private void appClose() {
		Preferences myPrefs =  Preferences.userNodeForPackage(UndoReDoFullWidow.class).node(this.getClass().getSimpleName());
	...
	private void initActions() {
       ...
       textPane.getActionMap().put("Undo",actionUndo);
       textPane.getInputMap().put(KeyStroke.getKeyStroke("control Z"), "Undo");
       
	    // Bind the undo action to ctl-y
       textPane.getActionMap().put("Redo",actionRedo);
       textPane.getInputMap().put(KeyStroke.getKeyStroke("control Y"), "Redo");
       
       textPane.getInputMap().put(KeyStroke.getKeyStroke("control M"), "cut");
       
	}//initActions

	public UndoReDoFullWidow() {
		initialize();
		appInit();
	}// Constructor
	...
	private void initialize() {
		...
		frmTemplate.setTitle("UndoReDoFullWidow    0.0");
		...
		Action action = textPane.getActionMap().get(DefaultEditorKit.cutAction);
		menuItem = mnuEdit.add(action);
		menuItem.setText("Cut");
		
		action = textPane.getActionMap().get(DefaultEditorKit.copyAction);
		menuItem = mnuEdit.add(action);
		menuItem.setText("Copy");
		
		action = textPane.getActionMap().get(DefaultEditorKit.pasteAction);
		menuItem = mnuEdit.add(action);
